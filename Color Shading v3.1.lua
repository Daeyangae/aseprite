...
local function colorShift(color, hueShift, satShift, lightShift, shadeShift, mixShift)
  local newColor = Color(color) -- Make a copy of the color so we don't modify the parameter
  local secColor = Color{red = app.bgColor.red, green = app.bgColor.green, blue = app.bgColor.blue}

  -- MIX COLORS
  if (mixShift > 0) then
    local newMix = Color {red = newColor.red, green = newColor.green, blue = newColor.blue}
    newMix.red = newMix.red - math.floor((newMix.red - secColor.red) * mixShift)
    newMix.green = newMix.green - math.floor((newMix.green - secColor.green) * mixShift)
    newMix.blue = newMix.blue - math.floor((newMix.blue - secColor.blue) * mixShift)
    newColor = newMix
  end
...
local function calculateColors(baseColor)
  ...
  S2 = colorShift(baseColor, 0, 0.2, -0.2, -0.3, 0);
  S3 = colorShift(baseColor, 0, 0.1, -0.1, -0.1, 0);
  S5 = colorShift(baseColor, 0, 0.1, 0.1, 0.1, 0);
  S6 = colorShift(baseColor, 0, 0.2, 0.2, 0.2, 0);
  S7 = colorShift(baseColor, 0, 0.3, 0.5, 0.4, 0);
  
  -- LIGHTNESS COLORS
  L1 = colorShift(baseColor, 0, 0, -0.4, 0, 0);
  L2 = colorShift(baseColor, 0, 0, -0.2, 0, 0);
  L3 = colorShift(baseColor, 0, 0, -0.1, 0, 0);
  L5 = colorShift(baseColor, 0, 0, 0.1, 0, 0);
  L6 = colorShift(baseColor, 0, 0, 0.2, 0, 0);
  L7 = colorShift(baseColor, 0, 0, 0.4, 0, 0);
  
  -- SATURATION COLORS
  C1 = colorShift(baseColor, 0, -0.5, 0, 0, 0);
  C2 = colorShift(baseColor, 0, -0.2, 0, 0, 0);
  C3 = colorShift(baseColor, 0, -0.1, 0, 0, 0);
  C5 = colorShift(baseColor, 0, 0.1, 0, 0, 0);
  C6 = colorShift(baseColor, 0, 0.2, 0, 0, 0);
  C7 = colorShift(baseColor, 0, 0.5, 0, 0, 0);
  
  -- HUE COLORS
  H1 = colorShift(baseColor, -0.15, 0, 0, 0, 0);
  H2 = colorShift(baseColor, -0.1, 0, 0, 0, 0);
  H3 = colorShift(baseColor, -0.05, 0, 0, 0, 0);
  H5 = colorShift(baseColor, 0.05, 0, 0, 0, 0);
  H6 = colorShift(baseColor, 0.1, 0, 0, 0, 0);
  H7 = colorShift(baseColor, 0.15, 0, 0, 0, 0);
